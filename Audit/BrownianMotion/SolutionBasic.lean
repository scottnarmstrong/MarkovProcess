...
This file is a verbatim copy of the vocabulary of `Challenge.lean` (everything but the
theorem), still Mathlib-only, so that `Solution.lean` can import the library and this
